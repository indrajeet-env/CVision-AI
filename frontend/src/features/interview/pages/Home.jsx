...
const Home = () => {
    ...
    const { loading, generateReport, reports, getReports, isHydrated } = useInterview()
    ...
    const handleGenerateReport = async () => {
        ...
        if (!resumeFile && !hasSelfDescription) {
        ...
        try {
            ...
            setError(err.response?.data?.message || err.message || "Failed to generate interview report. Please try again.");
        } finally {
            ...
            {reports && reports.length > 0 && (
                <aside className='home-sidebar'>
                    ...
                    <ul className='sidebar-reports-list'>
                        {reports.map(report => (
                            <li key={report._id} className='sidebar-report-item' onClick={() => navigate(`/interview/${report._id}`)}>
                                ...
                                <div className='resume-selected'>
                                    <div className='resume-item'>
                                        ...
                                        <div className='resume-info'>
                                            ...
                                            <p className='resume-size'>{(resumeFile.size / 1024).toFixed(2)} KB</p>
                                        </div>
                                    </div>
                                    <div className='resume-actions'>
                                        <button className='resume-btn resume-btn--preview' onClick={handlePreviewResume}>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" /></svg>
                                            Preview
                                        </button>
                                        <button className='resume-btn resume-btn--remove' onClick={handleRemoveResume}>
